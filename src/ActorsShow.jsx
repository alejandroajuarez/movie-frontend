export function ActorsShow({ actor, onUpddate, onDestroy }) {
  const handleSubmit = (event) => {
    event.preventDefault()
    console.log("Handling update");
    const params = new FormData(event.target)
    onUpddate(params, actor)
  }
  ...
  return (
    <div>
      <p>Actors</p>
      <p><b>id: </b>{actor.id}</p>
      <h2>Actor Name: {actor.first_name}{actor.last_name}</h2>
    ...
    <form onSubmit={handleSubmit}>
      ...
      <p>Movie ID: <input type="text" name="movie_id" defaultValue={actor.movie_id} /></p>
